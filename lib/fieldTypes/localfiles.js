...
localfiles.prototype.addToSchema = function() {
	...
	schema.virtual(paths.exists).get(function() {
		return this.get(field.path).some(function (f) { return schemaMethods.exists.apply(f); });
	});

	var reset = function(item) {
        item.set(paths.filename, '');
        item.set(paths.path, '');
        item.set(paths.size, 0);
        item.set(paths.filetype, '');
	};
	...
	var schemaMethods = {
		...
		delete: function(parent) {
            ...
            var fields = parent.get(field.path),
                idx = fields.indexOf(this);
            if (idx > -1) {
                fields.splice(idx, 1);
                parent.set(field.path, fields);
            }
		}
	};
	...
	return function() {
		...
		if (req.body && req.body[paths.action]) {
			...
			actions.forEach(function(action) {
				...
				var method = action[0],
					ids = action[1],
                    files = item.get(field.path);

				if (!(/^(delete|reset)$/.test(method)) || !ids) return;

				ids.split(',').forEach(function(id) {
                    var file;
                    if (files.some(function (f) { return f._id == id && (file = f); })) {
                        field.apply(file, method, item);
                    }
				});
			});
		}
		...
		// Upload new files
		if (req.files && req.files[paths.upload] && (req.files[paths.upload].length || req.files[paths.upload].size)) {
			return field.uploadFiles(
				item,
				Array.isArray(req.files[paths.upload]) ? req.files[paths.upload] : [req.files[paths.upload]],
				false,
				callback);
		}
